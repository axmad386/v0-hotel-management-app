...
export default function EditBookingPage({ params }: { params: { id: string } }) {
  ...
  async function onSubmit(data: BookingFormValues) {
    ...
    try {
      ...
      router.push(`/dashboard/bookings/(id)/${params.id}`)
    } catch (error) {
    ...
    <div className="container mx-auto py-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={() => router.push(`/dashboard/bookings/(id)/${params.id}`)}>
            Cancel
          </Button>
        </div>
      </div>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          ...
          <div className="flex justify-end gap-4">
            <Button
              type="button"
              variant="outline"
              onClick={() => router.push(`/dashboard/bookings/(id)/${params.id}`)}
              disabled={isSaving}
            >
              Cancel
            </Button>
