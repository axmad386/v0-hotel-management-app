...
export default function FinancialManagementPermissionsPage() {
  const router = useRouter()
  const [isLoading, setIsLoading] = useState(false)

  // Filter permissions related to financial management
  const financePermissions = PERMISSIONS.filter((p) => p.module === "finance")
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Success",
        description: "Financial management permissions updated successfully",
      })
    ...
    <div className="container mx-auto py-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Financial Management Permissions</h1>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <Card>
          <CardHeader>
            <CardTitle>Configure Financial Management Permissions</CardTitle>
            <CardDescription>Define which roles can access and manage financial features</CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
